defmodule Day11.Part1 do
  @doc """
      iex> Day11.Part1.part1("day11-sample.txt", 10, 10)
      37
  """
  def part1(filename, rows, cols) do
    filename
  ...
  @doc """
      iex> Day11.Part1.part1
      2093
  """
  def part1, do: part1("day11.txt", 91, 90)
  ...
  def next_state({col, row}, "L", map) do
    if Enum.all?(adjacent_seat_values({col, row}, map), &(!seat_occupied?(&1))),
      do: "#",
      else: "L"
  end

  def next_state({col, row}, "#", map) do
    if 4 <= Enum.count(adjacent_seat_values({col, row}, map), &seat_occupied?/1),
      do: "L",
      else: "#"
  end
  ...
  def adjacent_seat_values({col, row}, map) do
    adjacent_seats({col, row})
